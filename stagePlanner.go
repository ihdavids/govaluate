...
func findTypeChecks(symbol OperatorSymbol) typeChecks {
	...
	case GT:
		return typeChecks{
			combined: comparatorTypeCheck[OpGt],
		}
	case LT:
		return typeChecks{
			combined: comparatorTypeCheck[OpLt],
		}
	case GTE:
		return typeChecks{
			combined: comparatorTypeCheck[OpGte],
		}
	case LTE:
		return typeChecks{
			combined: comparatorTypeCheck[OpLte],
		}
	case REQ:
		return typeChecks{
			combined: regTypeCheck[OpNReg],
		}
	case NREQ:
		return typeChecks{
			combined: regTypeCheck[OpNReg],
	...
	case AND:
		return typeChecks{
			combined: boolTypeCheck[OpAnd],
		}
	case OR:
		return typeChecks{
			combined: boolTypeCheck[OpOr],
		}
	case IN:
		return typeChecks{
			combined: inTypeCheck[OpIn],
		}
	case BITWISE_LSHIFT:
		return typeChecks{
			combined: numericTypeCheck[OpLShft],
		}
	case BITWISE_RSHIFT:
		return typeChecks{
			combined: numericTypeCheck[OpRShft],
		}
	case BITWISE_OR:
		return typeChecks{
			combined: numericTypeCheck[OpBOr],
		}
	case BITWISE_AND:
		return typeChecks{
			combined: numericTypeCheck[OpBAnd],
		}
	case BITWISE_XOR:
		return typeChecks{
			combined: numericTypeCheck[OpBXor],
		}
	case PLUS:
		return typeChecks{
	...
	case MINUS:
		return typeChecks{
			combined: numericTypeCheck[OpSub],
		}
	case MULTIPLY:
		return typeChecks{
			combined: numericTypeCheck[OpMul],
		}
	case DIVIDE:
		return typeChecks{
			combined: numericTypeCheck[OpDiv],
		}
	case MODULUS:
		return typeChecks{
			combined: numericTypeCheck[OpMod],
		}
	case EXPONENT:
		return typeChecks{
			combined: numericTypeCheck[OpPow],
		}
	case NEGATE:
		return typeChecks{
			right: func(right interface{}) bool { return singleTypeCheck[OpNeg](right, isFloat64) },
		}
	case INVERT:
		return typeChecks{
			right: func(right interface{}) bool { return singleTypeCheck[OpInv](right, isBool) },
		}
	case BITWISE_NOT:
		return typeChecks{
			right: func(right interface{}) bool { return singleTypeCheck[OpBNot](right, isFloat64) },
		}
	case TERNARY_TRUE:
		return typeChecks{
			left: func(left interface{}) bool { return singleTypeCheck[OpTif](left, isBool) },
		}
