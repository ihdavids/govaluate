...
*/
func planStages(tokens []ExpressionToken, ops *OperatorOverloadMap) (*evaluationStage, error) {
...
*/
func elideLiterals(root *evaluationStage, ops *OperatorOverloadMap) *evaluationStage {

	if root.leftStage != nil {
		root.leftStage = elideLiterals(root.leftStage, ops)
	}

	if root.rightStage != nil {
		root.rightStage = elideLiterals(root.rightStage, ops)
	}

	return elideStage(root, ops)
}
...
*/
func elideStage(root *evaluationStage, ops *OperatorOverloadMap) *evaluationStage {
	...
	leftValue, err = root.leftStage.operator(ops, nil, nil, nil)
	if err != nil {
		return root
	}

	rightValue, err = root.rightStage.operator(ops, nil, nil, nil)
	if err != nil {
		return root
	}
	...
	result, err = root.operator(ops, leftValue, rightValue, nil)
	if err != nil {
		return root
	}
